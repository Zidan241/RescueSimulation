...
public class Simulator implements WorldListener {
	...
	private void loadBuildings(String path) throws Exception {
		...
		while (line != null) {
			...
			b.setBuildingButton(bu);
			ImageIcon icon1 = new ImageIcon("grass.png");
			JLabel background = new JLabel(icon1);
	        JLabel foreground = new JLabel(b.getCurrentIcon());
	        background.setLayout(new GridBagLayout());
	        background.add(foreground);
	        background.setVisible(true);
	        bu.add(background);
	        
			//bu.putClientProperty("Citizens", new ArrayList<Citizen>());
	...
	private void loadCitizens(String path) throws Exception {
		...
		while (line != null) {
			...
			JButton bu = new JButton(); 
			float[] hsb = Color.RGBtoHSB(82, 162, 16, null);
			 bu.setBorder(BorderFactory.createLineBorder(Color.getHSBColor(hsb[0],hsb[1],hsb[2]), 1));
